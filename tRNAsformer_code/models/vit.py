...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if self.have_pos_embed: self.pos_embed = nn.Parameter(torch.zeros(1, 1 + self.patch_embed.n_patches, embed_dim))
        ...
        if self.model_type == 'multitask': self.conv = nn.Conv1d(in_channels=embed_dim,
                                                                out_channels=n_genes,
                                                                kernel_size=1,
                                                                stride=1,
                                                                bias=True)
        if self.model_type == 'multitask': self.ks = [1, 2, 5, 10, 20, 49]
    ...
    def forward(self, x):
        ...
        if self.model_type == 'multitask':
            if self.training:
                ...
                gene_preds = 0
                for k in self.ks:
                    gene_preds += self.forward_fixed_k(x[:, 1:].transpose(2, 1), int(k)) / len(self.ks)
